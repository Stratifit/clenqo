...
**Established by:** `openspec/archive/create-scheduling-availability/` (implemented 2026-09-16, commit `ea8c08d`; hosted Supabase verification 12/12 PASS, full hosted suite 43/43 PASS)
**Sources:** `SCHEDULING_SYSTEM.md` §84–86, `DATABASE.md` §64/§65, `REQUIREMENTS.md` §18, `BOOKING_SYSTEM.md` §19, `SECURITY.md`, `PRICING_ENGINE.md` §13–14, `API_STANDARDS.md` §28
...
* Migration chain (0009) applies cleanly to a fresh database; constraints,
  uniquenesses, and RLS verified by automated tests (pglite) and applied to
  the hosted staging project.
* DST spring/autumn transitions and concurrency races covered by dedicated
  test cases (SCHEDULING_SYSTEM §76–78), locally and on hosted PostgreSQL.
* Hosted Supabase verification executed per tasks.md §13 (`HOSTED_VERIFY=1`):
  12/12 scheduling tests and 43/43 full hosted suite passed with zero
  leftover test data.
