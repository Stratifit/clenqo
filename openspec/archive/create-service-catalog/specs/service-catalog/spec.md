# Requirements: Service Catalog & Branch Service Configuration

**Change ID:** `create-service-catalog`

Capability: `service-catalog` (new). All requirements derive from
`docs/SERVICE_CATALOG.md` with approved decisions Q1–Q9 and the source
documents (`DATABASE.md` §15 pre-alignment, `SECURITY.md` §14,
`AUDIT_SYSTEM.md`, `API_STANDARDS.md`, `LOCALIZATION.md`,
`BOOKING_SYSTEM.md` §8–11). Requirement wording below is normative and
testable; WHERE/THEN scenarios are acceptance criteria.
...
* RLS/authorization verified as real authenticated contexts (unit-level
  emulation locally; hosted verification as a later task).
* Hosted Supabase verification is specified as a later implementation task
  and is NOT executed during design approval.
