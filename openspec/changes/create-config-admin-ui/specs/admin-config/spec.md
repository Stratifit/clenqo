# admin-config — Change 10 capability delta

> Proposed delta for the combined configuration-admin surface (Service
> Catalog, Pricing, Scheduling). The underlying domains (`service-catalog`,
> `pricing-engine`, `scheduling-availability`) remain authoritative and
> unchanged; this capability describes the staff-facing application surface
> that consumes them. Binding owner decisions: BD-E3a (one combined change),
> BD-E3b (scheduling mutations HQ-Admin-only via existing `branches.edit`),
> BD-E3d (no translation editing), BD-E3e (no seed-tool UI).
